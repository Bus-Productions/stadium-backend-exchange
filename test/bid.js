...
before(function(done) { helpers.post_symbol('BID', 100, 100, done) });

describe("Bid", function() {
  it('can post a bid', function(done) {
    request(app).post('/bid')
    //tock Ticker Symbol, Bid Amount, Bid Quantity, Buyer ID
      .send({ symbol: 'BID', price: 100.0, quantity: 100, buyer: 'Mr White' })
      .auth(USER, PASS)
      .expect(201)
  ...
  it('can get a bid', function(done) {
    request(app).get("/bid/1")
      ...
      .expect(200, function(err, res){
        res.body.symbol.should.equal('BID');
  ...
  describe('failure scenarios', function() {
    ...
    it('needs a bid quantity', function(done) {
      request(app).post('/bid')
        .send({ symbol: 'BID', price: 100.0, buyer: 'Mr White' })
        .auth(USER, PASS)
    ...
    it('needs a bid price', function(done) {
      request(app).post('/bid')
        .send({ symbol: 'BID', quantity: 100, buyer: 'Mr White' })
        .auth(USER, PASS)
        .expect(400, function(err, res){
    ...
    it('needs a bid buyer', function(done) {
      request(app).post('/bid')
        .send({ symbol: 'BID', price: 100.0, quantity: 100 })
        .auth(USER, PASS)
        .expect(400, function(err, res){
